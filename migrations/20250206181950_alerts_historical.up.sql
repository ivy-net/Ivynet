...
BEGIN
    FOR org_id IN SELECT organization_id FROM organization LOOP
        EXECUTE format('CREATE TABLE IF NOT EXISTS alerts_historical_%s PARTITION OF alerts_historical FOR VALUES IN (%s);', org_id, org_id);
    END LOOP;
END $$;

-- Create the partition tables for new organizations
CREATE OR REPLACE FUNCTION create_alerts_historical_partition()
RETURNS trigger AS $$
DECLARE
    partition_name text;
BEGIN
    partition_name := format('alerts_historical_%s', NEW.organization_id);

    EXECUTE format(
       'CREATE TABLE IF NOT EXISTS %I PARTITION OF alerts_historical FOR VALUES IN (%L);',
       partition_name,
       NEW.organization_id
    );
...
CREATE TRIGGER after_insert_organization_historical
AFTER INSERT ON organization
FOR EACH ROW
EXECUTE FUNCTION create_alerts_historical_partition();
