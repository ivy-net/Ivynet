...
pub trait OrganizationDatabase: Send + Sync + 'static {
...
impl NotificationDispatcher {
    pub fn new(config: NotificationConfig, db: Box<dyn OrganizationDatabase>) -> Self {
        let db = Arc::new(db);

        Self {
            telegram: TelegramBot::new(config.telegram_token, db.clone()),
