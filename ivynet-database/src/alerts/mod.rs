...
mod test_alerts_db {
    ...
    use node::alerts_active::NewNodeAlert;
    use sqlx::PgPool;
    use uuid::Uuid;
    ...
    async fn test_add_new_alert(pool: PgPool) {
        let alerts_active = node::alerts_active::NodeActiveAlert::get_all(&pool).await.unwrap();
        ...
        node::alerts_active::NodeActiveAlert::insert_one(&pool, &new_alert).await.unwrap();
        ...
        let new_db_alert =
            node::alerts_active::NodeActiveAlert::get(&pool, new_alert.id).await.unwrap().unwrap();
    ...
    async fn test_get_all_active_alerts(pool: PgPool) {
        let alert = node::alerts_active::NodeActiveAlert::get_all(&pool).await.unwrap();
    ...
    async fn test_get_alert_by_id(pool: PgPool) {
        let alert = node::alerts_active::NodeActiveAlert::get(&pool, debug_uuid()).await.unwrap();
    ...
    async fn test_get_alerts_by_org(pool: PgPool) {
        let alert =
            node::alerts_active::NodeActiveAlert::all_alerts_by_org(&pool, 1).await.unwrap();
        assert!(!alert.is_empty());
    }
    ...
    async fn test_get_alerts_by_machine(pool: PgPool) {
        ...
        let alert =
            node::alerts_active::NodeActiveAlert::all_alerts_by_machine(&pool, fixture_machine_id)
                .await
                .unwrap();
        assert!(!alert.is_empty());
    }
    ...
    async fn test_acknowledge_alert(pool: PgPool) {
        let alert =
            node::alerts_active::NodeActiveAlert::get(&pool, debug_uuid()).await.unwrap().unwrap();
        node::alerts_active::NodeActiveAlert::acknowledge(&pool, alert.alert_id).await.unwrap();
    ...
    async fn test_resolve_alert(pool: PgPool) {
        let alert =
            node::alerts_active::NodeActiveAlert::get(&pool, debug_uuid()).await.unwrap().unwrap();
        node::alerts_active::NodeActiveAlert::resolve_alert(&pool, alert.alert_id).await.unwrap();

        // confirm that the alert is resolved in historical db
        let alert_historical =
            node::alerts_historical::NodeHistoryAlert::get(&pool, debug_uuid()).await.unwrap();
        assert!(alert_historical.is_some());

        // confirm that the alert is removed from active db
        let alert_resolved =
            node::alerts_active::NodeActiveAlert::get(&pool, debug_uuid()).await.unwrap();
