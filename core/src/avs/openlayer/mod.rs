...
pub const OPENLAYER_PATH: &str = ".eigenlayer/openlayer";
...
impl OpenLayer {
    ...
    pub fn new_from_chain(chain: Chain) -> Self {
        ...
        Self::new(home_dir.join(OPENLAYER_PATH), chain)
    }
}

impl Default for OpenLayer {
    fn default() -> Self {
        let home_dir = dirs::home_dir().unwrap();
        Self::new(home_dir.join(OPENLAYER_PATH), Chain::Holesky)
    }
}

#[cfg_attr(target_arch = "wasm32", async_trait(?Send))]
#[cfg_attr(not(target_arch = "wasm32"), async_trait)]
impl AvsVariant for OpenLayer {
    async fn setup(
        &self,
        ...
        // TODO: Currently OpenLayer works only on Holesky. But we don't pass any parameters
        // stating what network it will be ran against. We need to do something about it
        let _ = Command::new("docker")
            .arg("pull")
            .arg("gcr.io/openoracle-de73b/operator-js-holesky:latest")
            .status()?;
    ...
    async fn start(&mut self, quorums: Vec<QuorumType>, chain: Chain) -> Result<Child, IvyError> {
        ...
        if !build.success() {
            return Err(OpenLayerError::ScriptError(build.to_string()).into());
        }
    ...
    fn registry_coordinator(&self, chain: Chain) -> Address {
        match chain {
            Chain::Holesky => h160!(0xE0315CCaF46A736BFAB173670CBcC97bE65Eb414),
            _ => todo!("Unimplemented"),
        }
    }
    ...
    async fn register(
        ...
        if optin.success() {
            ...
            Err(OpenLayerError::ScriptError(optin.to_string()).into())
        }
    }
    ...
    async fn unregister(
        ...
        todo!("OpenLayer has no unregister function");
    }
    ...
    async fn build_env(
        ...
        // Node resource paths
        env_lines.set("NODE_G1_PATH_HOST", r#"${EIGENLAYER_HOME}/openlayer/resources/g1.point"#);
        env_lines.set(
            "NODE_G2_PATH_HOST",
            r#"${EIGENLAYER_HOME}/openlayer/resources/g2.point.powerOf2"#,
        );
        env_lines.set(
            "NODE_CACHE_PATH_HOST",
            r#"${EIGENLAYER_HOME}/openlayer/eigenda-operator-setup/resources/cache"#,
        );
