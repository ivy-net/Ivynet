...
fn create_router() -> Router<HttpState> {
    Router::new()
        ...
        .route("/client/status", get(machine::status))
        .route("/client/idle", get(machine::idling))
        .route("/client/unhealthy", get(machine::unhealthy))
        .route("/client/healthy", get(machine::healthy))
        .route("/client/:id/metrics", get(machine::metrics_condensed))
        .route("/client/:id/metrics/all", get(machine::metrics_all))
        .route("/client/:id/logs", get(machine::logs))
        .route("/client/:id/data", get(machine::get_all_node_data))
        // .route("/client/:id/data/:avs", get(client::get_avs_data))
        .route("/client/:id/data", delete(machine::delete_machine_data))
        // .route("/client/:id/data/:avs/:operator_id", delete(client::delete_avs_data))
        .route("/client/:id", get(machine::info))
        .route("/client/:id", post(machine::set_name))
        .route("/client/:id", delete(machine::delete))
