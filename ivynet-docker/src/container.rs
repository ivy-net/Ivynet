...
use bollard::{
    ...
    errors::Error,
    secret::{ContainerInspectResponse, ContainerState, HostConfig, ImageInspect},
};
...
impl FullContainer {
    pub fn new(container: ContainerInspectResponse, image: ImageInspect) -> Self {
        Self { container, image }
    }

    pub fn state(&self) -> Option<&ContainerState> {
        self.container.state.as_ref()
    }
    ...
    pub fn id(&self) -> Option<&str> {
        self.container.id.as_deref()
    }

    /// Image ID for the associated container. This is the image ID, NOT THE DIGEST. For the
    /// digest, use `repo_digest`.
    pub fn image_id(&self) -> Option<&str> {
        self.container.image.as_deref()
    }

    /// `repo@digest` for the associated container. Can be decomposed into `repo` and `digest`.
    pub fn repo_digest(&self) -> Option<RepoDigest> {
        let digests = self.image.repo_digests.clone()?;
    ...
    pub fn repo_tag(&self) -> Option<RepoTag> {
        ...
        RepoTag::from_str(tag_str).ok()
    }

    pub fn repo(&self) -> Option<String> {
        Some(self.repo_digest()?.image)
    }

    pub fn digest(&self) -> Option<String> {
        Some(self.repo_digest()?.digest?.to_string())
    }

    pub fn ports(&self) -> Option<Vec<ExposedPort>> {
        let ports: Vec<ExposedPort> = self
            .container
            .clone()
            .config?
            .exposed_ports?
            .keys()
            .filter_map(|k| ExposedPort::from_str(k).ok())
            .collect();
        Some(ports)
    }
    ...
    fn is_network_mode_host(&self) -> bool {
        if let Some(host_config) = &self.container.host_config {
